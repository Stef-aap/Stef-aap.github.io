.ORG    $9D93
.DB     $0BB, $6D

Start:
    LD (__RETPointer), SP
_HomeUp:  .equ  $4558
_AbsO1O2Cp:  .equ  $410E
_AbsO1PAbsO2:  .equ  $405A
_ACos:  .equ  $40DE
_ACosH:  .equ  $40F0
_ACosRad:  .equ  $40D2
_AdrLEle:  .equ  $462D
_AdrMEle:  .equ  $4609
_AdrMRow:  .equ  $4606
_AllEq:  .equ  $4876
_AllocFPS:  .equ  $43A5
_AllocFPS1:  .equ  $43A8
_Angle:  .equ  $4102
_AnsName:  .equ  $4B52
_ApdSetup:  .equ  $4C93
_AppGetCalc:  .equ  $4C78
_AppGetCbl:  .equ  $4C75
_AppInit:  .equ  $404B
_Arc_Unarc:  .equ  $4FD8
_ASin:  .equ  $40E4
_ASinH:  .equ  $40ED
_ASinRad:  .equ  $40DB
_ATan:  .equ  $40E1
_ATan2:  .equ  $40E7
_ATan2Rad:  .equ  $40D8
_ATanH:  .equ  $40EA
_ATanRad:  .equ  $40D5
_BinOPExec:  .equ  $4663
_Bit_VertSplit:  .equ  $4FA8
_BufClr:  .equ  $5074
_BufCpy:  .equ  $5071
_CAbs:  .equ  $4E97
_CAdd:  .equ  $4E88
_CanAlphIns:  .equ  $4C69
_CDiv:  .equ  $4E94
_CDivByReal:  .equ  $4EBB
_CEtoX:  .equ  $4EA9
_CFrac:  .equ  $4EC1
_CheckSplitFlag:  .equ  $49F0
_Chk_Batt_Low:  .equ  $50B3
_ChkFindSym:  .equ  $42F1
_CIntgr:  .equ  $4EC4
_CircCmd:  .equ  $47D4
_CkInt:  .equ  $4234
_CkOdd:  .equ  $4237
_CkOP1C0:  .equ  $4225
_CkOP1Cplx:  .equ  $40FC
_CkOP1FP0:  .equ  $4228
_CkOP1Pos:  .equ  $4258
_CkOP1Real:  .equ  $40FF
_CkOP2FP0:  .equ  $422B
_CkOP2Pos:  .equ  $4255
_CkOP2Real:  .equ  $42DF
_CkPosInt:  .equ  $4231
_CkValidNum:  .equ  $4270
_CleanAll:  .equ  $4A50
_ClearRect:  .equ  $4D5C
_ClearRow:  .equ  $4CED
_CLine:  .equ  $4798
_CLineS:  .equ  $479B
_CLN:  .equ  $4EA0
_CLog:  .equ  $4EA3
_CloseEditBuf:  .equ  $48D3
_CloseEditBufNoR:  .equ  $476E
_CloseEditEqu:  .equ  $496C
_CloseProg:  .equ  $4A35
_ClrGraphRef:  .equ  $4A38
_ClrLCD:  .equ  $4543
_ClrLCDFull:  .equ  $4540
_ClrLp:  .equ  $41D1
_ClrOP1S:  .equ  $425E
_ClrOP2S:  .equ  $425B
_ClrScrn:  .equ  $4549
_ClrScrnFull:  .equ  $4546
_ClrTxtShd:  .equ  $454C
_CMltByReal:  .equ  $4EB8
_CmpSyms:  .equ  $4A4A
_CMult:  .equ  $4E8E
_Conj:  .equ  $4EB5
_ConvDim:  .equ  $4B43
_ConvDim00:  .equ  $4B46
_ConvLcToLr:  .equ  $4A23
_ConvLrToLc:  .equ  $4A56
_ConvOP1:  .equ  $4AEF
_COP1Set0:  .equ  $4105
_Cos:  .equ  $40C0
_CosH:  .equ  $40CC
_CpHLDE:  .equ  $400C
_CPoint:  .equ  $4DC8
_CPointS:  .equ  $47F5
_CpOP1OP2:  .equ  $4111
_CpOP4OP3:  .equ  $4108
_CpyO1ToFPS1:  .equ  $445C
_CpyO1ToFPS2:  .equ  $446B
_CpyO1ToFPS3:  .equ  $4477
_CpyO1ToFPS4:  .equ  $4489
_CpyO1ToFPS5:  .equ  $4483
_CpyO1ToFPS6:  .equ  $447D
_CpyO1ToFPS7:  .equ  $4480
_CpyO1ToFPST:  .equ  $444A
_CpyO2ToFPS1:  .equ  $4459
_CpyO2ToFPS2:  .equ  $4462
_CpyO2ToFPS3:  .equ  $4474
_CpyO2ToFPS4:  .equ  $4486
_CpyO2ToFPST:  .equ  $4444
_CpyO3ToFPS1:  .equ  $4453
_CpyO3ToFPS2:  .equ  $4465
_CpyO3ToFPST:  .equ  $4441
_CpyO5ToFPS1:  .equ  $4456
_CpyO5ToFPS3:  .equ  $4471
_CpyO6ToFPS2:  .equ  $4468
_CpyO6ToFPST:  .equ  $4447
_CpyStack:  .equ  $4429
_CpyTo1FPS1:  .equ  $4432
_CpyTo1FPS10:  .equ  $43F3
_CpyTo1FPS11:  .equ  $43D8
_CpyTo1FPS2:  .equ  $443B
_CpyTo1FPS3:  .equ  $4408
_CpyTo1FPS4:  .equ  $440E
_CpyTo1FPS5:  .equ  $43DE
_CpyTo1FPS6:  .equ  $43E4
_CpyTo1FPS7:  .equ  $43EA
_CpyTo1FPS8:  .equ  $43ED
_CpyTo1FPS9:  .equ  $43F6
_CpyTo1FPST:  .equ  $4423
_CpyTo2FPS1:  .equ  $442F
_CpyTo2FPS2:  .equ  $4438
_CpyTo2FPS3:  .equ  $4402
_CpyTo2FPS4:  .equ  $43F9
_CpyTo2FPS5:  .equ  $43DB
_CpyTo2FPS6:  .equ  $43E1
_CpyTo2FPS7:  .equ  $43E7
_CpyTo2FPS8:  .equ  $43F0
_CpyTo2FPST:  .equ  $4420
_CpyTo3FPS1:  .equ  $442C
_CpyTo3FPS2:  .equ  $4411
_CpyTo3FPST:  .equ  $441D
_CpyTo4FPST:  .equ  $441A
_CpyTo5FPST:  .equ  $4414
_CpyTo6FPS2:  .equ  $43FF
_CpyTo6FPS3:  .equ  $43FC
_CpyTo6FPST:  .equ  $4417
_CpyToFPS1:  .equ  $445F
_CpyToFPS2:  .equ  $446E
_CpyToFPS3:  .equ  $447A
_CpyToFPST:  .equ  $444D
_CpyToStack:  .equ  $4450
_Create0Equ:  .equ  $432A
_CreateAppVar:  .equ  $4E6A
_CreateCList:  .equ  $431B
_CreateCplx:  .equ  $430C
_CreateEqu:  .equ  $4330
_CreatePair:  .equ  $4B0D
_CreatePict:  .equ  $4333
_CreateProg:  .equ  $4339
_CreateProtProg:  .equ  $4E6D
_CreateReal:  .equ  $430F
_CreateRList:  .equ  $4315
_CreateRMat:  .equ  $4321
_CreateStrng:  .equ  $4327
_CRecip:  .equ  $4E91
_CSqRoot:  .equ  $4E9D
_CSquare:  .equ  $4E8B
_CSub:  .equ  $4E85
_CTenX:  .equ  $4EA6
_CTrunc:  .equ  $4EBE
_Cube:  .equ  $407B
_CursorOff:  .equ  $45BE
_CursorOn:  .equ  $45C4
_CXrootY:  .equ  $4EAC
_CYtoX:  .equ  $4EB2
_DarkLine:  .equ  $47DD
_DarkPnt:  .equ  $47F2
_DataSize:  .equ  $436C
_DataSizeA:  .equ  $4369
_DeallocFPS:  .equ  $439F
_DeallocFPS1:  .equ  $43A2
_DecO1Exp:  .equ  $4267
_DelListEl:  .equ  $4A2F
_DelMem:  .equ  $4357
_DelRes:  .equ  $4A20
_DelVar:  .equ  $4351
_DelVarArc:  .equ  $4FC6
_DelVarNoArc:  .equ  $4FC9
_DisableApd:  .equ  $4C84
_Disp:  .equ  $4F45
_DispDone:  .equ  $45B5
_DispEOL:  .equ  $45A6
_DispHL:  .equ  $4507
_DisplayImage:  .equ  $4D9B
_DispMenuTitle:  .equ  $5065
_DispOP1A:  .equ  $4BF7
_DivHLBy10:  .equ  $400F
_DivHLByA:  .equ  $4012
_DrawCirc2:  .equ  $4C66
_DrawCmd:  .equ  $48C1
_DrawRectBorder:  .equ  $4D7D
_DrawRectBorderClear:  .equ  $4D8C
_DToR:  .equ  $4075
_EditProg:  .equ  $4A32
_EnableApd:  .equ  $4C87
_EnoughMem:  .equ  $42FD
_EOP1NotReal:  .equ  $4279
_Equ_or_NewEqu:  .equ  $42C4
_EraseEOL:  .equ  $4552
_EraseRectBorder:  .equ  $4D86
_ErrArgument:  .equ  $44AD
_ErrBadGuess:  .equ  $44CB
_ErrBreak:  .equ  $44BF
_ErrD_OP1_0:  .equ  $42D3
_ErrD_OP1_LE_0:  .equ  $42D0
_ErrD_OP1Not_R:  .equ  $42CA
_ErrD_OP1NotPos:  .equ  $42C7
_ErrD_OP1NotPosInt:  .equ  $42CD
_ErrDataType:  .equ  $44AA
_ErrDimension:  .equ  $44B3
_ErrDimMismatch:  .equ  $44B0
_ErrDivBy0:  .equ  $4498
_ErrDomain:  .equ  $449E
_ErrIncrement:  .equ  $44A1
_ErrInvalid:  .equ  $44BC
_ErrIterations:  .equ  $44C8
_ErrLinkXmit:  .equ  $44D4
_ErrMemory:  .equ  $44B9
_ErrNon_Real:  .equ  $44A4
_ErrNonReal:  .equ  $4A8C
_ErrNotEnoughMem:  .equ  $448C
_ErrOverflow:  .equ  $4495
_ErrSignChange:  .equ  $44C5
_ErrSingularMat:  .equ  $449B
_ErrStat:  .equ  $44C2
_ErrStatPlot:  .equ  $44D1
_ErrSyntax:  .equ  $44A7
_ErrTolTooSmall:  .equ  $44CE
_ErrUndefined:  .equ  $44B6
_EToX:  .equ  $40B4
_Exch9:  .equ  $43D5
_ExLp:  .equ  $4222
_ExpToHex:  .equ  $424F
_Factorial:  .equ  $4B85
_FillRect:  .equ  $4D62
_FillRectPattern:  .equ  $4D89
_Find_Parse_Formula:  .equ  $4AF2
_FindAlphaDn:  .equ  $4A47
_FindAlphaUp:  .equ  $4A44
_FindApp:  .equ  $4C4E
_FindAppDn:  .equ  $4C4B
_FindAppNumPages:  .equ  $509B
_FindAppUp:  .equ  $4C48
_FindSym:  .equ  $42F4
_FiveExec:  .equ  $467E
_FixTempCnt:  .equ  $4A3B
_FlashToRam:  .equ  $5017
_ForceFullScreen:  .equ  $508F
_FormBase:  .equ  $50AA
_FormDCplx:  .equ  $4996
_FormEReal:  .equ  $4990
_FormReal:  .equ  $4999
_FourExec:  .equ  $467B
_FPAdd:  .equ  $4072
_FPDiv:  .equ  $4099
_FPMult:  .equ  $4084
_FPRecip:  .equ  $4096
_FPSquare:  .equ  $4081
_FPSub:  .equ  $406F
_Frac:  .equ  $4093
_Get_Tok_Strng:  .equ  $4594
_GetBaseVer:  .equ  $4C6F
_GetCSC:  .equ  $4018
_GetKey:  .equ  $4972
_GetLToOP1:  .equ  $4636
_GetMToOP1:  .equ  $4615
_GetSysInfo:  .equ  $50DD
_GetTokLen:  .equ  $4591
_GrBufClr:  .equ  $4BD0
_GrBufCpy:  .equ  $486A
_GrphCirc:  .equ  $47D7
_HLTimes9:  .equ  $40F9
_HorizCmd:  .equ  $48A6
_HTimesL:  .equ  $4276
_IBounds:  .equ  $4C60
_IBoundsFull:  .equ  $4D98
_ILine:  .equ  $47E0
_IncLstSize:  .equ  $4A29
_InsertList:  .equ  $4A2C
_InsertMem:  .equ  $42F7
_Int:  .equ  $40A5
_Intgr:  .equ  $405D
_InvCmd:  .equ  $48C7
_InvertRect:  .equ  $4D5F
_InvOP1S:  .equ  $408D
_InvOP1SC:  .equ  $408A
_InvOP2S:  .equ  $4090
_InvSub:  .equ  $4063
_IOffset:  .equ  $4C63
_IPoint:  .equ  $47E3
_IsA2ByteTok:  .equ  $42A3
_IsEditEmpty:  .equ  $492D
_JError:  .equ  $44D7
_JErrorNo:  .equ  $4000
_JForceCmdNoChar:  .equ  $4027
_JForceGraphKey:  .equ  $5005
_JForceGraphNoKey:  .equ  $5002
_KeyToString:  .equ  $45CA
_LCD_BUSY:  .equ  $4051
_LCD_DRIVERON:  .equ  $4978
_LdHLind:  .equ  $4009
_LineCmd:  .equ  $48AC
_LnX:  .equ  $40AB
_Load_SFont:  .equ  $4783
_LoadCIndPaged:  .equ  $501D
_LoadDEIndPaged:  .equ  $501A
_LoadPattern:  .equ  $4CB1
_LogX:  .equ  $40AE
_Max:  .equ  $4057
_MemChk:  .equ  $42E5
_MemClear:  .equ  $4C30
_MemSet:  .equ  $4C33
_Min:  .equ  $4054
_Minus1:  .equ  $406C
_Mov10B:  .equ  $415C
_Mov18B:  .equ  $47DA
_Mov7B:  .equ  $4168
_Mov8B:  .equ  $4165
_Mov9B:  .equ  $415F
_Mov9OP1OP2:  .equ  $417D
_Mov9OP2Cp:  .equ  $410B
_Mov9ToOP1:  .equ  $417A
_Mov9ToOP2:  .equ  $4180
_MovFrOP1:  .equ  $4183
_NewLine:  .equ  $452E
_NZIf83Plus:  .equ  $50E0
_OneVar:  .equ  $4BA3
_OP1ExOP2:  .equ  $421F
_OP1ExOP3:  .equ  $4219
_OP1ExOP4:  .equ  $421C
_OP1ExOP5:  .equ  $420D
_OP1ExOP6:  .equ  $4210
_OP1ExpToDec:  .equ  $4252
_OP1Set0:  .equ  $41BF
_OP1Set1:  .equ  $419B
_OP1Set2:  .equ  $41A7
_OP1Set3:  .equ  $41A1
_OP1Set4:  .equ  $419E
_OP1ToOP2:  .equ  $412F
_OP1ToOP3:  .equ  $4123
_OP1ToOP4:  .equ  $4117
_OP1ToOP5:  .equ  $4153
_OP1ToOP6:  .equ  $4150
_OP2ExOP4:  .equ  $4213
_OP2ExOP5:  .equ  $4216
_OP2ExOP6:  .equ  $4207
_OP2Set0:  .equ  $41BC
_OP2Set1:  .equ  $41AD
_OP2Set2:  .equ  $41AA
_OP2Set3:  .equ  $4198
_OP2Set4:  .equ  $4195
_OP2Set5:  .equ  $418F
_OP2Set60:  .equ  $4AB0
_OP2Set8:  .equ  $418C
_OP2SetA:  .equ  $4192
_OP2ToOP1:  .equ  $4156
_OP2ToOP3:  .equ  $416E
_OP2ToOP4:  .equ  $411A
_OP2ToOP5:  .equ  $414A
_OP2ToOP6:  .equ  $414D
_OP3Set0:  .equ  $41B9
_OP3Set1:  .equ  $4189
_OP3Set2:  .equ  $41A4
_OP3ToOP1:  .equ  $413E
_OP3ToOP2:  .equ  $4120
_OP3ToOP4:  .equ  $4114
_OP3ToOP5:  .equ  $4147
_OP4Set0:  .equ  $41B6
_OP4Set1:  .equ  $4186
_OP4ToOP1:  .equ  $4138
_OP4ToOP2:  .equ  $411D
_OP4ToOP3:  .equ  $4171
_OP4ToOP5:  .equ  $4144
_OP4ToOP6:  .equ  $4177
_OP5ExOP6:  .equ  $420A
_OP5Set0:  .equ  $41B3
_OP5ToOP1:  .equ  $413B
_OP5ToOP2:  .equ  $4126
_OP5ToOP3:  .equ  $4174
_OP5ToOP4:  .equ  $412C
_OP5ToOP6:  .equ  $4129
_OP6ToOP1:  .equ  $4135
_OP6ToOP2:  .equ  $4132
_OP6ToOP5:  .equ  $4141
_OutputExpr:  .equ  $4BB2
_PagedGet:  .equ  $5023
_ParseInp:  .equ  $4A9B
_PDspGrph:  .equ  $48A3
_PixelTest:  .equ  $48B5
_Plus1:  .equ  $4069
_PointCmd:  .equ  $48B2
_PointOn:  .equ  $4C39
_PopMCplxO1:  .equ  $436F
_PopOP1:  .equ  $437E
_PopOP3:  .equ  $437B
_PopOP5:  .equ  $4378
_PopReal:  .equ  $4393
_PopRealO1:  .equ  $4390
_PopRealO2:  .equ  $438D
_PopRealO3:  .equ  $438A
_PopRealO4:  .equ  $4387
_PopRealO5:  .equ  $4384
_PopRealO6:  .equ  $4381
_PosNo0Int:  .equ  $422E
_PToR:  .equ  $40F3
_PushMCplxO1:  .equ  $43CF
_PushMCplxO3:  .equ  $43C6
_PushOP1:  .equ  $43C9
_PushOP3:  .equ  $43C3
_PushOP5:  .equ  $43C0
_PushReal:  .equ  $43BD
_PushRealO1:  .equ  $43BA
_PushRealO2:  .equ  $43B7
_PushRealO3:  .equ  $43B4
_PushRealO4:  .equ  $43B1
_PushRealO5:  .equ  $43AE
_PushRealO6:  .equ  $43AB
_PutAway:  .equ  $403c
_PutC:  .equ  $4504
_PutMap:  .equ  $4501
_PutPS:  .equ  $4510
_PutPSB:  .equ  $450d
_PutS:  .equ  $450A
_PutTokString:  .equ  $4960
_PutToL:  .equ  $4645
_PutToMat:  .equ  $461E
_RandInit:  .equ  $4B7F
_Random:  .equ  $4B79
_Rcl_StatVar:  .equ  $42DC
_RclAns:  .equ  $4AD7
_RclGDB2:  .equ  $47D1
_RclN:  .equ  $4ADD
_RclSysTok:  .equ  $4AE6
_RclVarSym:  .equ  $4AE3
_RclX:  .equ  $4AE0
_RclY:  .equ  $4ADA
_Rec1stByte:  .equ  $4EFA
_Rec1stByteNC:  .equ  $4EFD
_RecAByteIO:  .equ  $4F03
_RedimMat:  .equ  $4A26
_Regraph:  .equ  $488E
_ReleaseBuffer:  .equ  $4771
_ReloadAppEntryVecs:  .equ  $4C36
_RestoreDisp:  .equ  $4870
_RName:  .equ  $427F
_RndGuard:  .equ  $409F
_RnFx:  .equ  $40A2
_Round:  .equ  $40A8
_RToD:  .equ  $4078
_RToP:  .equ  $40F6
_RunIndicOff:  .equ  $4570
_RunIndicOn:  .equ  $456D
_SaveDisp:  .equ  $4C7B
_SendAByte:  .equ  $4EE5
_SendVarCmd:  .equ  $4A14
_SetAllPlots:  .equ  $4FCC
_SetExSpeed:  .equ  $50BF
_SetFuncM:  .equ  $4840
_SetNorm_Vals:  .equ  $49FC
_SetParM:  .equ  $4849
_SetPolM:  .equ  $4846
_SetSeqM:  .equ  $4843
_SetTblGraphDraw:  .equ  $4C00
_SetupPagedPtr:  .equ  $5020
_SetXXOP1:  .equ  $478C
_SetXXOP2:  .equ  $478F
_SetXXXXOP2:  .equ  $4792
_SFont_Len:  .equ  $4786
_ShRAcc:  .equ  $41D4
_Sin:  .equ  $40BD
_SinCosRad:  .equ  $40BA
_SinH:  .equ  $40CF
_SinHCosH:  .equ  $40C6
_SqRoot:  .equ  $409C
_SrchVLstDn:  .equ  $4F12
_SrchVLstUp:  .equ  $4F0F
_SStringLength:  .equ  $4CB4
_StMatEl:  .equ  $4AE9
_StoAns:  .equ  $4ABF
_StoGDB2:  .equ  $47CE
_StoN:  .equ  $4ACB
_StoOther:  .equ  $4AD4
_StoR:  .equ  $4AC5
_StoRand:  .equ  $4B7C
_StoSysTok:  .equ  $4ABC
_StoT:  .equ  $4ACE
_StoTheta:  .equ  $4AC2
_StoX:  .equ  $4AD1
_StoY:  .equ  $4AC8
_StrCopy:  .equ  $44E3
_StrLength:  .equ  $4C3F
_Tan:  .equ  $40C3
_TanH:  .equ  $40C9
_TanLnF:  .equ  $48BB
_TenX:  .equ  $40B7
_ThetaName:  .equ  $427C
_ThreeExec:  .equ  $4675
_Times2:  .equ  $4066
_TimesPt5:  .equ  $407E
_TName:  .equ  $428E
_ToFrac:  .equ  $4657
_Trunc:  .equ  $4060
_TwoVarSet:  .equ  $50D4
_UCLineS:  .equ  $4795
_UnLineCmd:  .equ  $48AF
_UnOPExec:  .equ  $4672
_VertCmd:  .equ  $48A9
_VPutMap:  .equ  $455E
_VPutS:  .equ  $4561
_VPutSN:  .equ  $4564
_VtoWHLDE:  .equ  $47FB
_XftoI:  .equ  $4804
_Xitof:  .equ  $47FE
_XName:  .equ  $4288
_XRootY:  .equ  $479E
_YftoI:  .equ  $4801
_YName:  .equ  $428B
_YToX:  .equ  $47A1
_Zero16D:  .equ  $41B0
_ZeroOP:  .equ  $41CE
_ZeroOP1:  .equ  $41C5
_ZeroOP2:  .equ  $41C8
_ZeroOP3:  .equ  $41CB
_ZmDecml:  .equ  $484F
_ZmFit:  .equ  $485B
_ZmInt:  .equ  $484C
_ZmPrev:  .equ  $4852
_ZmSquare:  .equ  $485E
_ZmStats:  .equ  $47A4
_ZmTrig:  .equ  $4861
_ZmUsr:  .equ  $4855
_ZooDefault:  .equ  $4867
XSCLt:  .equ  2
YSCLt:  .equ  3
XMINt:  .equ  10
XMAXt:  .equ  11
YMINt:  .equ  12
YMAXt:  .equ  13
TMINt:  .equ  14
TMAXt:  .equ  15
THETMINt:  .equ  16
THETMAXt:  .equ  17
TBLMINt:  .equ  26
PLOTSTARTt:  .equ  27
NMAXt:  .equ  29
NMINt:  .equ  31
TBLSTEPt:  .equ  33
TSTEPt:  .equ  34
THETSTEPt:  .equ  35
DELTAXt:  .equ  38
DELTAYt:  .equ  39
XFACTt:  .equ  40
YFACTt:  .equ  41
FINNt:  .equ  43
FINIt:  .equ  44
FINPVt:  .equ  45
FINPMTt:  .equ  46
FINFVt:  .equ  47
FINPYt:  .equ  48
FINCYt:  .equ  49
PLOTSTEPt:  .equ  52
XRESt:  .equ  54
busyPause:  .equ  170
busyNormal:  .equ  240
rOP1TOOP2:  .equ  8
rFINDSYM:  .equ  16
rPUSHREALO1:  .equ  24
rMOV9TOOP1:  .equ  32
rFPADD:  .equ  48
ramStart:  .equ  32768
appData:  .equ  32768
ramCode:  .equ  33024
ramCodeEnd:  .equ  33327
baseAppBrTab:  .equ  33328
bootTemp:  .equ  33361
appSearchPage:  .equ  33443
tempSwapArea:  .equ  33445
appID:  .equ  33677
ramReturnData:  .equ  33773
arcInfo:  .equ  33774
savedArcInfo:  .equ  33798
appInfo:  .equ  33842
appBank_jump:  .equ  33852
appPage:  .equ  33854
kbdScanCode:  .equ  33855
kbdKey:  .equ  33860
kbdGetKy:  .equ  33861
keyExtend:  .equ  33862
contrast:  .equ  33863
apdSubTimer:  .equ  33864
apdTimer:  .equ  33865
curTime:  .equ  33866
curRow:  .equ  33867
curCol:  .equ  33868
curOffset:  .equ  33869
curUnder:  .equ  33870
curY:  .equ  33871
curType:  .equ  33872
curXRow:  .equ  33873
prevDData:  .equ  33874
lFont_record:  .equ  33882
sFont_record:  .equ  33890
tokVarPtr:  .equ  33898
tokLen:  .equ  33900
indicMem:  .equ  33902
indicCounter:  .equ  33910
indicBusy:  .equ  33911
OP1:  .equ  33912
OP1M:  .equ  33914
OP2:  .equ  33923
OP2M:  .equ  33925
OP2EXT:  .equ  33932
OP3:  .equ  33934
OP3M:  .equ  33936
OP4:  .equ  33945
OP4M:  .equ  33947
OP5:  .equ  33956
OP5M:  .equ  33958
OP6:  .equ  33967
OP6M:  .equ  33969
OP6EXT:  .equ  33976
progToEdit:  .equ  33983
nameBuff:  .equ  33991
equ_edit_save:  .equ  34002
iMathPtr1:  .equ  34003
iMathPtr2:  .equ  34005
iMathPtr3:  .equ  34007
iMathPtr4:  .equ  34009
iMathPtr5:  .equ  34011
chkDelPtr1:  .equ  34013
chkDelPtr2:  .equ  34015
insDelPtr:  .equ  34017
upDownPtr:  .equ  34019
fOutDat:  .equ  34021
asm_data_ptr1:  .equ  34027
asm_data_ptr2:  .equ  34029
asm_sym_ptr1:  .equ  34031
asm_sym_ptr2:  .equ  34033
asm_ram:  .equ  34035
asm_ind_call:  .equ  34055
textShadow:  .equ  34056
textShadCur:  .equ  34184
textShadTop:  .equ  34186
textShadAlph:  .equ  34187
textShadIns:  .equ  34188
cxMain:  .equ  34189
cxPPutAway:  .equ  34191
cxPutAway:  .equ  34193
cxRedisp:  .equ  34195
cxErrorEP:  .equ  34197
cxSizeWind:  .equ  34199
cxPage:  .equ  34201
cxCurApp:  .equ  34202
cxPrev:  .equ  34203
monQH:  .equ  34218
monQT:  .equ  34219
monQueue:  .equ  34220
onSP:  .equ  34236
promptRow:  .equ  34240
promptCol:  .equ  34241
promptIns:  .equ  34242
promptShift:  .equ  34243
promptRet:  .equ  34244
promptValid:  .equ  34246
promptTop:  .equ  34248
promptCursor:  .equ  34250
promptTail:  .equ  34252
promptBtm:  .equ  34254
varType:  .equ  34256
varCurrent:  .equ  34257
varClass:  .equ  34265
menuActive:  .equ  34268
menuAppDepth:  .equ  34269
MenuCurrent:  .equ  34270
ProgCurrent:  .equ  34280
userMenuSA:  .equ  34302
ioPrompt:  .equ  34399
dImageWidth:  .equ  34400
RectFillPHeight:  .equ  34400
RectFillPWidth:  .equ  34401
RectFillPattern:  .equ  34402
ioFlag:  .equ  34416
sndRecState:  .equ  34418
ioErrState:  .equ  34419
header:  .equ  34420
ioData:  .equ  34429
ioNewData:  .equ  34441
bakHeader:  .equ  34443
penCol:  .equ  34519
penRow:  .equ  34520
rclQueue:  .equ  34521
rclQueueEnd:  .equ  34523
errNo:  .equ  34525
errSP:  .equ  34526
errOffset:  .equ  34528
saveSScreen:  .equ  34540
bstCounter:  .equ  35310
flags:  .equ  35312
statVars:  .equ  35386
anovaf_vars:  .equ  35863
infVars:  .equ  35917
curGStyle:  .equ  36119
curGY:  .equ  36120
curGX:  .equ  36121
curGY2:  .equ  36122
curGX2:  .equ  36123
freeSaveY:  .equ  36124
freeSaveX:  .equ  36125
XOffset:  .equ  36257
YOffset:  .equ  36258
lcdTallP:  .equ  36259
pixWideP:  .equ  36260
pixWide_m_1:  .equ  36261
pixWide_m_2:  .equ  36262
lastEntryPTR:  .equ  36263
lastEntryStk:  .equ  36265
numLastEntries:  .equ  36393
currLastEntry:  .equ  36394
curInc:  .equ  36455
uXmin:  .equ  36478
uXmax:  .equ  36487
uXscl:  .equ  36496
uYmin:  .equ  36505
uYmax:  .equ  36514
uYscl:  .equ  36523
uThetMin:  .equ  36532
uThetMax:  .equ  36541
uThetStep:  .equ  36550
uTmin:  .equ  36559
uTmax:  .equ  36568
uTStep:  .equ  36577
uPlotStart:  .equ  36586
unMax:  .equ  36595
uu0:  .equ  36604
uv0:  .equ  36613
unMin:  .equ  36622
uu02:  .equ  36631
uv02:  .equ  36640
uw0:  .equ  36649
uPlotStep:  .equ  36658
uXres:  .equ  36667
uw02:  .equ  36676
Xmin:  .equ  36688
Xmax:  .equ  36697
Xscl:  .equ  36706
Ymin:  .equ  36715
Ymax:  .equ  36724
Yscl:  .equ  36733
ThetaMin:  .equ  36742
ThetaMax:  .equ  36751
ThetaStep:  .equ  36760
TminPar:  .equ  36769
TmaxPar:  .equ  36778
Tstep:  .equ  36787
PlotStart:  .equ  36796
nMax:  .equ  36805
u0:  .equ  36814
v0:  .equ  36823
nMin:  .equ  36832
u02:  .equ  36841
v02:  .equ  36850
w0:  .equ  36859
PlotStep:  .equ  36868
XresO:  .equ  36877
w02:  .equ  36886
un1:  .equ  36895
un2:  .equ  36904
vn1:  .equ  36913
vn2:  .equ  36922
wn1:  .equ  36931
wn2:  .equ  36940
fin_N:  .equ  36949
fin_I:  .equ  36958
fin_PV:  .equ  36967
fin_PMT:  .equ  36976
fin_FV:  .equ  36985
fin_PY:  .equ  36994
fin_CY:  .equ  37003
cal_N:  .equ  37012
cal_I:  .equ  37021
cal_PV:  .equ  37030
cal_PMT:  .equ  37039
cal_FV:  .equ  37048
cal_PY:  .equ  37057
smallEditRAM:  .equ  37075
XFact:  .equ  37183
YFact:  .equ  37192
Xres_int:  .equ  37201
deltaX:  .equ  37202
deltaY:  .equ  37211
shortX:  .equ  37220
shortY:  .equ  37229
lower:  .equ  37238
upper:  .equ  37247
XOutSym:  .equ  37260
XOutDat:  .equ  37262
YOutSym:  .equ  37264
YOutDat:  .equ  37266
inputSym:  .equ  37268
inputDat:  .equ  37270
prevData:  .equ  37272
TblMin:  .equ  37555
TblStep:  .equ  37564
P1Type:  .equ  37577
SavX1List:  .equ  37578
SavY1List:  .equ  37583
SavF1List:  .equ  37588
P1FrqOnOff:  .equ  37593
P2Type:  .equ  37594
SavX2List:  .equ  37595
SavY2List:  .equ  37600
SavF2List:  .equ  37605
P2FrqOnOff:  .equ  37610
P3Type:  .equ  37611
SavX3List:  .equ  37612
SavY3List:  .equ  37617
SavF3List:  .equ  37622
P3FrqOnOff:  .equ  37627
plotSScreen:  .equ  37696
seed1:  .equ  38464
seed2:  .equ  38473
cmdShadow:  .equ  38510
cmdShadCur:  .equ  38638
cmdShadAlph:  .equ  38640
cmdShadIns:  .equ  38641
cmdCursor:  .equ  38642
editTop:  .equ  38644
editCursor:  .equ  38646
editTail:  .equ  38648
editBtm:  .equ  38650
editSym:  .equ  38662
editDat:  .equ  38664
GY0:  .equ  38783
GY1:  .equ  38774
GY2:  .equ  38775
GY3:  .equ  38776
GY4:  .equ  38777
GY5:  .equ  38778
GY6:  .equ  38779
GY7:  .equ  38780
GY8:  .equ  38781
GY9:  .equ  38782
GX1:  .equ  38784
GX2:  .equ  38785
GX3:  .equ  38786
GX4:  .equ  38787
GX5:  .equ  38788
GX6:  .equ  38789
GU:  .equ  38796
GV:  .equ  38797
GW:  .equ  38798
GR1:  .equ  38790
GR2:  .equ  38791
GR3:  .equ  38792
GR4:  .equ  38793
GR5:  .equ  38794
GR6:  .equ  38795
winTop:  .equ  38821
winBtm:  .equ  38822
winLeftEdge:  .equ  38823
winLeft:  .equ  38824
winAbove:  .equ  38826
winRow:  .equ  38828
winCol:  .equ  38830
fmtDigits:  .equ  38832
fmtString:  .equ  38833
fmtConv:  .equ  38898
fmtLeft:  .equ  38916
fmtIndex:  .equ  38918
fmtMatSym:  .equ  38920
fmtMatMem:  .equ  38922
EQS:  .equ  38924
tSymPtr1:  .equ  38936
tSymPtr2:  .equ  38938
chkDelPtr3:  .equ  38940
chkDelPtr4:  .equ  38942
tempMem:  .equ  38944
fpBase:  .equ  38946
FPS:  .equ  38948
OPBase:  .equ  38950
OPS:  .equ  38952
pTempCnt:  .equ  38954
cleanTmp:  .equ  38956
pTemp:  .equ  38958
progPtr:  .equ  38960
newDataPtr:  .equ  38962
pagedCount:  .equ  38964
pagedPN:  .equ  38965
pagedGetPtr:  .equ  38966
pagedPutPtr:  .equ  38968
pagedBuf:  .equ  38970
appErr1:  .equ  38989
appErr2:  .equ  39002
flashByte1:  .equ  39015
flashByte2:  .equ  39016
freeArcBlock:  .equ  39017
arcPage:  .equ  39019
arcPtr:  .equ  39020
appRawKeyHandle:  .equ  39024
appBackUpScreen:  .equ  39026
customHeight:  .equ  39794
localLanguage:  .equ  39795
cursorHookPtr:  .equ  39804
rawKeyHookPtr:  .equ  39812
getKeyHookPtr:  .equ  39816
fontHookPtr:  .equ  39836
restartClr:  .equ  39888
localTokStr:  .equ  40293
keyForStr:  .equ  40310
keyToStrRam:  .equ  40311
sedMonSp:  .equ  40328
bpSave:  .equ  40330
userMem:  .equ  40341
symTable:  .equ  65126
LANG_NEUTRAL:  .equ  0
LANG_DANISH:  .equ  6
LANG_DUTCH:  .equ  19
LANG_ENGLISH:  .equ  9
LANG_FINNISH:  .equ  11
LANG_FRENCH:  .equ  12
LANG_GERMAN:  .equ  7
LANG_HUNGARIAN:  .equ  14
LANG_ITALIAN:  .equ  16
LANG_NORWEGIAN:  .equ  20
LANG_POLISH:  .equ  21
LANG_PORTUGUESE:  .equ  22
LANG_SPANISH:  .equ  10
LANG_SWEDISH:  .equ  29
SUBLANG_NEUTRAL:  .equ  0
SUBLANG_DUTCH:  .equ  1
SUBLANG_ENGLISH_US:  .equ  1
SUBLANG_FRENCH:  .equ  1
SUBLANG_GERMAN:  .equ  1
SUBLANG_ITALIAN:  .equ  1
SUBLANG_NORWEGIAN_BOKMAL:  .equ  1
SUBLANG_PORTUGUESE:  .equ  2
SUBLANG_SPANISH:  .equ  1
trigFlags:  .equ  0
trigDeg:  .equ  2
kbdFlags:  .equ  0
kbdSCR:  .equ  3
kbdKeyPress:  .equ  4
doneFlags:  .equ  0
donePrgm:  .equ  5
ioDelFlag:  .equ  0
inDelete:  .equ  0
editFlags:  .equ  1
editOpen:  .equ  2
monFlags:  .equ  1
monAbandon:  .equ  4
plotFlags:  .equ  2
plotTrace:  .equ  0
plotLoc:  .equ  1
plotDisp:  .equ  2
grfModeFlags:  .equ  2
grfFuncM:  .equ  4
grfPolarM:  .equ  5
grfParamM:  .equ  6
grfRecurM:  .equ  7
graphFlags:  .equ  3
graphDraw:  .equ  0
grfDBFlags:  .equ  4
grfDot:  .equ  0
grfSimul:  .equ  1
grfGrid:  .equ  2
grfPolar:  .equ  3
grfNoCoord:  .equ  4
grfNoAxis:  .equ  5
grfLabel:  .equ  6
textFlags:  .equ  5
textEraseBelow:  .equ  1
textScrolled:  .equ  2
textInverse:  .equ  3
textInsMode:  .equ  4
ParsFlag2:  .equ  7
numOP1:  .equ  0
newDispF:  .equ  8
preClrForMode:  .equ  0
apdFlags:  .equ  8
apdAble:  .equ  2
apdRunning:  .equ  3
web_err_mask:  .equ  96
onFlags:  .equ  9
onRunning:  .equ  3
onInterrupt:  .equ  4
statFlags:  .equ  9
statsValid:  .equ  6
statANSDISP:  .equ  7
fmtFlags:  .equ  10
fmtExponent:  .equ  0
fmtEng:  .equ  1
fmtHex:  .equ  2
fmtOct:  .equ  3
fmtBin:  .equ  4
numMode:  .equ  10
fmtReal:  .equ  5
fmtRect:  .equ  6
fmtPolar:  .equ  7
realMode:  .equ  5
rectMode:  .equ  6
polarMode:  .equ  7
fmtBaseMask:  .equ  28
fmtBaseShift:  .equ  2
fmtOverride:  .equ  11
fmtEditFlags:  .equ  12
fmtEdit:  .equ  0
curFlags:  .equ  12
curAble:  .equ  2
curOn:  .equ  3
curLock:  .equ  4
appFlags:  .equ  13
appWantIntrpt:  .equ  0
appTextSave:  .equ  1
appAutoScroll:  .equ  2
appMenus:  .equ  3
appLockMenus:  .equ  4
appCurGraphic:  .equ  5
appCurWord:  .equ  6
appExit:  .equ  7
seqFlags:  .equ  15
webMode:  .equ  0
webVert:  .equ  1
sequv:  .equ  2
seqvw:  .equ  3
sequw:  .equ  4
promptFlags:  .equ  17
promptEdit:  .equ  0
indicFlags:  .equ  18
indicRun:  .equ  0
indicInUse:  .equ  1
indicOnly:  .equ  2
shiftFlags:  .equ  18
shift2nd:  .equ  3
shiftAlpha:  .equ  4
shiftLwrAlph:  .equ  5
shiftALock:  .equ  6
shiftKeepAlph:  .equ  7
tblFlags:  .equ  19
autoFill:  .equ  4
autoCalc:  .equ  5
reTable:  .equ  6
sGrFlags:  .equ  20
grfSplit:  .equ  0
vertSplit:  .equ  1
grfSChanged:  .equ  2
grfSplitOverride:  .equ  3
write_on_graph:  .equ  4
g_style_active:  .equ  5
cmp_mod_box:  .equ  6
textWrite:  .equ  7
newIndicFlags:  .equ  21
extraIndic:  .equ  0
saIndic:  .equ  1
newFlags2:  .equ  22
noRestores:  .equ  5
smartFlags:  .equ  23
smarter_mask:  .equ  3
smarter_test:  .equ  1
smartGraph:  .equ  0
smartGraph_inv:  .equ  1
more_Flags:  .equ  26
No_Del_Stat:  .equ  2
asm_Flag1:  .equ  33
asm_Flag2:  .equ  34
asm_Flag3:  .equ  35
getSendFlg:  .equ  36
comFailed:  .equ  1
appLwrCaseFlag:  .equ  36
lwrCaseActive:  .equ  3
apiFlg3:  .equ  42
apiFlg4:  .equ  43
fullScrnDraw:  .equ  2
groupFlags:  .equ  38
inGroup:  .equ  1
xapFlag0:  .equ  46
xapFlag1:  .equ  47
xapFlag2:  .equ  48
xapFlag3:  .equ  49
fontFlags:  .equ  50
fracDrawLFont:  .equ  2
fracTallLFont:  .equ  3
customFont:  .equ  7
plotFlag3:  .equ  60
bufferOnly:  .equ  0
useFastCirc:  .equ  4
DBKeyFlags:  .equ  61
repeatMost:  .equ  4
haveDBKey:  .equ  5
keyDefaultsF:  .equ  6
HWLinkErrF:  .equ  7
varTypeMask:  .equ  31
varGraphRef:  .equ  6
LrecurN:  .equ  1
LrecurU:  .equ  2
LrecurV:  .equ  3
LrecurW:  .equ  4
Lconvert:  .equ  5
LsqUp:  .equ  6
LsqDown:  .equ  7
Lintegral:  .equ  8
Lcross:  .equ  9
LboxIcon:  .equ  10
LcrossIcon:  .equ  11
LdotIcon:  .equ  12
LsubT:  .equ  13
LcubeR:  .equ  14
LhexF:  .equ  15
Lroot:  .equ  16
Linverse:  .equ  17
Lsquare:  .equ  18
Langle:  .equ  19
Ldegree:  .equ  20
Lradian:  .equ  21
Ltranspose:  .equ  22
LLE:  .equ  23
LNE:  .equ  24
LGE:  .equ  25
Lneg:  .equ  26
Lexponent:  .equ  27
Lstore:  .equ  28
Lten:  .equ  29
LupArrow:  .equ  30
LdownArrow:  .equ  31
Lspace:  .equ  32
Lexclam:  .equ  33
Lquote:  .equ  34
Lpound:  .equ  35
Lfourth:  .equ  36
Lpercent:  .equ  37
Lampersand:  .equ  38
Lapostrophe:  .equ  39
LlParen:  .equ  40
LrParen:  .equ  41
Lasterisk:  .equ  42
LplusSign:  .equ  43
Lcomma:  .equ  44
Ldash:  .equ  45
Lperiod:  .equ  46
Lslash:  .equ  47
L0:  .equ  48
L1:  .equ  49
L2:  .equ  50
L3:  .equ  51
L4:  .equ  52
L5:  .equ  53
L6:  .equ  54
L7:  .equ  55
L8:  .equ  56
L9:  .equ  57
Lcolon:  .equ  58
Lsemicolon:  .equ  59
LLT:  .equ  60
LEQ:  .equ  61
LGT:  .equ  62
Lquestion:  .equ  63
LatSign:  .equ  64
LcapA:  .equ  4
LcapB:  .equ  4
LcapC:  .equ  67
LcapD:  .equ  68
LcapE:  .equ  69
LcapF:  .equ  70
LcapG:  .equ  71
LcapH:  .equ  72
LcapI:  .equ  73
LcapJ:  .equ  74
LcapK:  .equ  75
LcapL:  .equ  76
LcapM:  .equ  77
LcapN:  .equ  78
LcapO:  .equ  79
LcapP:  .equ  80
LcapQ:  .equ  81
LcapR:  .equ  82
LcapS:  .equ  83
LcapT:  .equ  84
LcapU:  .equ  85
LcapV:  .equ  86
LcapW:  .equ  87
LcapX:  .equ  88
LcapY:  .equ  89
LcapZ:  .equ  90
Ltheta:  .equ  91
Lbackslash:  .equ  92
LrBrack:  .equ  93
Lcaret:  .equ  94
Lunderscore:  .equ  95
Lbackquote:  .equ  96
La:  .equ  97
Lb:  .equ  98
Lc:  .equ  99
Ld:  .equ  100
Le:  .equ  101
Lf:  .equ  102
Lg:  .equ  103
Lh:  .equ  104
Li:  .equ  105
Lj:  .equ  106
Lk:  .equ  107
Ll:  .equ  108
Lm:  .equ  109
Ln:  .equ  110
Lo:  .equ  111
Lp:  .equ  112
Lq:  .equ  113
Lr:  .equ  114
Ls:  .equ  115
Lt:  .equ  116
Lu:  .equ  117
Lv:  .equ  118
Lw:  .equ  119
Lx:  .equ  120
Ly:  .equ  121
Lz:  .equ  122
LlBrace:  .equ  123
Lbar:  .equ  124
LrBrace:  .equ  125
Ltilde:  .equ  126
LinvEQ:  .equ  127
Lsub0:  .equ  128
Lsub1:  .equ  129
Lsub2:  .equ  130
Lsub3:  .equ  131
Lsub4:  .equ  132
Lsub5:  .equ  133
Lsub6:  .equ  134
Lsub7:  .equ  135
Lsub8:  .equ  136
Lsub9:  .equ  137
LcapAAcute:  .equ  138
LcapAGrave:  .equ  139
LcapACaret:  .equ  140
LcapADier:  .equ  141
LaAcute:  .equ  142
LaGrave:  .equ  143
LaCaret:  .equ  144
LaDier:  .equ  145
LcapEAcute:  .equ  146
LcapEGrave:  .equ  147
LcapECaret:  .equ  148
LcapEDier:  .equ  149
LeAcute:  .equ  150
LeGrave:  .equ  151
LeCaret:  .equ  152
LeDier:  .equ  153
LcapIAcute:  .equ  154
LcapIGrave:  .equ  155
LcapICaret:  .equ  156
LcapIDier:  .equ  157
LiAcute:  .equ  158
LiGrave:  .equ  159
LiCaret:  .equ  160
LiDier:  .equ  161
LcapOAcute:  .equ  162
LcapOGrave:  .equ  163
LcapOCaret:  .equ  164
LcapODier:  .equ  165
LoAcute:  .equ  166
LoGrave:  .equ  167
LoCaret:  .equ  168
LoDier:  .equ  169
LcapUAcute:  .equ  170
LcapUGrave:  .equ  171
LcapUCaret:  .equ  172
LcapUDier:  .equ  173
LuAcute:  .equ  174
LuGrave:  .equ  175
LuCaret:  .equ  176
LuDier:  .equ  177
LcapCCed:  .equ  178
LcCed:  .equ  179
LcapNTilde:  .equ  180
LnTilde:  .equ  181
Laccent:  .equ  182
Lgrave:  .equ  183
Ldieresis:  .equ  184
LquesDown:  .equ  185
LexclamDown:  .equ  186
Lalpha:  .equ  187
Lbeta:  .equ  188
Lgamma:  .equ  189
LcapDelta:  .equ  190
Ldelta:  .equ  191
Lepsilon:  .equ  192
LlBrack:  .equ  193
Llambda:  .equ  194
Lmu:  .equ  195
Lpi:  .equ  196
Lrho:  .equ  197
LcapSigma:  .equ  198
Lsigma:  .equ  199
Ltau:  .equ  200
Lphi:  .equ  201
LcapOmega:  .equ  202
LxMean:  .equ  203
LyMean:  .equ  204
LsupX:  .equ  205
Lellipsis:  .equ  206
Lleft:  .equ  207
Lblock:  .equ  208
Lper:  .equ  209
Lhyphen:  .equ  210
Larea:  .equ  211
Ltemp:  .equ  212
Lcube:  .equ  213
Lenter:  .equ  214
LimagI:  .equ  215
Lphat:  .equ  216
Lchi:  .equ  217
LstatF:  .equ  218
Llne:  .equ  219
LlistL:  .equ  220
LfinanN:  .equ  221
L2_r_paren:  .equ  222
LblockArrow:  .equ  223
LcurO:  .equ  224
LcurO2:  .equ  225
LcurOcapA:  .equ  226
LcurOa:  .equ  227
LcurI:  .equ  228
LcurI2:  .equ  229
LcurIcapA:  .equ  230
LcurIa:  .equ  231
LGline:  .equ  232
LGthick:  .equ  233
LGabove:  .equ  234
LGbelow:  .equ  235
LGpath:  .equ  236
LGanimate:  .equ  237
LGdot:  .equ  238
LUpBlk:  .equ  239
LDnBlk:  .equ  240
LcurFull:  .equ  241
SrecurN:  .equ  1
SrecurU:  .equ  2
SrecurV:  .equ  3
SrecurW:  .equ  4
Sconvert:  .equ  5
SFourSpaces:  .equ  6
SsqDown:  .equ  7
Sintegral:  .equ  8
Scross:  .equ  9
SboxIcon:  .equ  10
ScrossIcon:  .equ  11
SdotIcon:  .equ  12
SsubT:  .equ  13
ScubeR:  .equ  14
ShexF:  .equ  15
Sroot:  .equ  16
Sinverse:  .equ  17
Ssquare:  .equ  18
Sangle:  .equ  19
Sdegree:  .equ  20
Sradian:  .equ  21
Stranspose:  .equ  22
SLE:  .equ  23
SNE:  .equ  24
SGE:  .equ  25
Sneg:  .equ  26
Sexponent:  .equ  27
Sstore:  .equ  28
Sten:  .equ  29
SupArrow:  .equ  30
SdownArrow:  .equ  31
Sspace:  .equ  32
Sexclam:  .equ  33
Squote:  .equ  34
Spound:  .equ  35
Sdollar:  .equ  36
Spercent:  .equ  37
Sampersand:  .equ  38
Sapostrophe:  .equ  39
SlParen:  .equ  40
SrParen:  .equ  41
Sasterisk:  .equ  42
SplusSign:  .equ  43
Scomma:  .equ  44
Sdash:  .equ  45
Speriod:  .equ  46
Sslash:  .equ  47
S0:  .equ  48
S1:  .equ  49
S2:  .equ  50
S3:  .equ  51
S4:  .equ  52
S5:  .equ  53
S6:  .equ  54
S7:  .equ  55
S8:  .equ  56
S9:  .equ  57
Scolon:  .equ  58
Ssemicolon:  .equ  59
SLT:  .equ  60
SEQ:  .equ  61
SGT:  .equ  62
Squestion:  .equ  63
SatSign:  .equ  64
ScapA:  .equ  65
ScapB:  .equ  66
ScapC:  .equ  67
ScapD:  .equ  68
ScapE:  .equ  69
ScapF:  .equ  70
ScapG:  .equ  71
ScapH:  .equ  72
ScapI:  .equ  73
ScapJ:  .equ  74
ScapK:  .equ  75
ScapL:  .equ  76
ScapM:  .equ  77
ScapN:  .equ  78
ScapO:  .equ  79
ScapP:  .equ  80
ScapQ:  .equ  81
ScapR:  .equ  82
ScapS:  .equ  83
ScapT:  .equ  84
ScapU:  .equ  85
ScapV:  .equ  86
ScapW:  .equ  87
ScapX:  .equ  88
ScapY:  .equ  89
ScapZ:  .equ  90
Stheta:  .equ  91
Sbackslash:  .equ  92
SrBrack:  .equ  93
Scaret:  .equ  94
Sunderscore:  .equ  95
Sbackquote:  .equ  96
SmallA:  .equ  97
SmallB:  .equ  98
SmallC:  .equ  99
SmallD:  .equ  100
SmallE:  .equ  101
SmallF:  .equ  102
SmallG:  .equ  103
SmallH:  .equ  104
SmallI:  .equ  105
SmallJ:  .equ  106
SmallK:  .equ  107
SmallL:  .equ  108
SmallM:  .equ  109
SmallN:  .equ  110
SmallO:  .equ  111
SmallP:  .equ  112
SmallQ:  .equ  113
SmallR:  .equ  114
SmallS:  .equ  115
SmallT:  .equ  116
SmallU:  .equ  117
SmallV:  .equ  118
SmallW:  .equ  119
SmallX:  .equ  120
SmallY:  .equ  121
SmallZ:  .equ  122
SlBrace:  .equ  123
Sbar:  .equ  124
SrBrace:  .equ  125
Stilde:  .equ  126
SinvEQ:  .equ  127
Ssub0:  .equ  128
Ssub1:  .equ  129
Ssub2:  .equ  130
Ssub3:  .equ  131
Ssub4:  .equ  132
Ssub5:  .equ  133
Ssub6:  .equ  134
Ssub7:  .equ  135
Ssub8:  .equ  136
Ssub9:  .equ  137
ScapAAcute:  .equ  138
ScapAGrave:  .equ  139
ScapACaret:  .equ  140
ScapADier:  .equ  141
SaAcute:  .equ  142
SaGrave:  .equ  143
SaCaret:  .equ  144
SaDier:  .equ  145
ScapEGrave:  .equ  146
ScapEAcute:  .equ  147
ScapECaret:  .equ  148
ScapEDier:  .equ  149
SeAcute:  .equ  150
SeGrave:  .equ  151
SeCaret:  .equ  152
SeDier:  .equ  153
ScapIAcute:  .equ  154
ScapIGrave:  .equ  155
ScapICaret:  .equ  156
ScapIDier:  .equ  157
SiAcute:  .equ  158
SiGrave:  .equ  159
SiCaret:  .equ  160
SiDier:  .equ  161
ScapOAcute:  .equ  162
ScapOGrave:  .equ  163
ScapOCaret:  .equ  164
ScapODier:  .equ  165
SoAcute:  .equ  166
SoGrave:  .equ  167
SoCaret:  .equ  168
SoDier:  .equ  169
ScapUAcute:  .equ  170
ScapUGrave:  .equ  171
ScapUCaret:  .equ  172
ScapUDier:  .equ  173
SuAcute:  .equ  174
SuGrave:  .equ  175
SuCaret:  .equ  176
SuDier:  .equ  177
ScapCCed:  .equ  178
ScCed:  .equ  179
ScapNTilde:  .equ  180
SnTilde:  .equ  181
Saccent:  .equ  182
Sgrave:  .equ  183
Sdieresis:  .equ  184
SquesDown:  .equ  185
SexclamDown:  .equ  186
Salpha:  .equ  187
Sbeta:  .equ  188
Sgamma:  .equ  189
ScapDelta:  .equ  190
Sdelta:  .equ  191
Sepsilon:  .equ  192
SlBrack:  .equ  193
Slambda:  .equ  194
Smu:  .equ  195
Spi:  .equ  196
Srho:  .equ  197
ScapSigma:  .equ  198
Ssigma:  .equ  199
Stau:  .equ  200
Sphi:  .equ  201
ScapOmega:  .equ  202
SxMean:  .equ  203
SyMean:  .equ  204
SsupX:  .equ  205
Sellipsis:  .equ  206
Sleft:  .equ  207
Sblock:  .equ  208
Sper:  .equ  209
Shyphen:  .equ  210
Sarea:  .equ  211
Stemp:  .equ  212
Scube:  .equ  213
Senter:  .equ  214
SimagI:  .equ  215
Sphat:  .equ  216
Schi:  .equ  217
SstatF:  .equ  218
Slne:  .equ  219
SlistL:  .equ  220
SfinanN:  .equ  221
S2_r_paren:  .equ  222
SnarrowCapE:  .equ  223
SListLock:  .equ  224
Sscatter1:  .equ  225
Sscatter2:  .equ  226
Sxyline1:  .equ  227
Sxyline2:  .equ  228
Sboxplot1:  .equ  229
Sboxplot2:  .equ  230
Shist1:  .equ  231
Shist2:  .equ  232
SmodBox1:  .equ  233
SmodBox2:  .equ  234
Snormal1:  .equ  235
Snormal2:  .equ  236
kRight:  .equ  1
kLeft:  .equ  2
kUp:  .equ  3
kDown:  .equ  4
kEnter:  .equ  5
kAlphaEnter:  .equ  6
kAlphaUp:  .equ  7
kAlphaDown:  .equ  8
kClear:  .equ  9
kDel:  .equ  10
kIns:  .equ  11
kRecall:  .equ  12
kLastEnt:  .equ  13
kBOL:  .equ  14
kEOL:  .equ  15
kUnselAll:  .equ  17
kLtoTI82:  .equ  18
kBackup:  .equ  19
kRecieve:  .equ  20
kLnkQuit:  .equ  21
kTrans:  .equ  22
kRename:  .equ  23
kOverw:  .equ  24
kOmit:  .equ  25
kCont:  .equ  26
kSendID:  .equ  27
kSendSW:  .equ  28
kYes:  .equ  29
kNoWay:  .equ  30
kvSendType:  .equ  31
kOverWAll:  .equ  32
kKReset:  .equ  38
kApp:  .equ  39
kDoug:  .equ  40
kListflag:  .equ  41
menuStart:  .equ  43
kAppsMenu:  .equ  44
kPrgm:  .equ  45
kZoom:  .equ  46
kDraw:  .equ  47
kSPlot:  .equ  48
kStat:  .equ  49
kMath:  .equ  50
kTest:  .equ  51
kChar:  .equ  52
kVars:  .equ  53
kMem:  .equ  54
kMatrix:  .equ  55
kDist:  .equ  56
kAngle:  .equ  57
kList:  .equ  58
kCalc:  .equ  59
kFin:  .equ  60
menuEnd:  .equ  60
kCatalog:  .equ  62
kInputDone:  .equ  63
kOff:  .equ  63
kQuit:  .equ  64
appStart:  .equ  64
kLinkIO:  .equ  65
kMatrixEd:  .equ  66
kStatEd:  .equ  67
kGraph:  .equ  68
kMode:  .equ  69
kPrgmEd:  .equ  70
kPrgmCr:  .equ  71
kWindow:  .equ  72
kYequ:  .equ  73
kTable:  .equ  74
kTblSet:  .equ  75
kChkRAM:  .equ  76
kDelMem:  .equ  77
kResetMem:  .equ  78
kResetDef:  .equ  79
kPrgmInput:  .equ  80
kZFactEd:  .equ  81
kError:  .equ  82
kSolveTVM:  .equ  83
kSolveRoot:  .equ  84
kStatP:  .equ  85
kInfStat:  .equ  86
kFormat:  .equ  87
kExtApps:  .equ  88
kNewApps:  .equ  89
append:  .equ  89
echoStart1:  .equ  90
kTrace:  .equ  90
kZFit:  .equ  91
kZIn:  .equ  92
kZOut:  .equ  93
kZPrev:  .equ  94
kBox:  .equ  95
kDecml:  .equ  96
kSetZm:  .equ  97
kSquar:  .equ  98
kStd:  .equ  99
kTrig:  .equ  100
kUsrZm:  .equ  101
kZSto:  .equ  102
kZInt:  .equ  103
kZStat:  .equ  104
echoStart2:  .equ  105
kSelect:  .equ  105
kCircl:  .equ  106
kClDrw:  .equ  107
kLine:  .equ  108
kPen:  .equ  109
kPtChg:  .equ  110
kPtOff:  .equ  111
kPtOn:  .equ  112
kVert:  .equ  113
kHoriz:  .equ  114
kText:  .equ  115
kTanLn:  .equ  116
kEval:  .equ  117
kInters:  .equ  118
kDYDX:  .equ  119
kFnIntg:  .equ  120
kRootG:  .equ  121
kDYDT:  .equ  122
kDXDT:  .equ  123
kDRDo:  .equ  124
kGFMin:  .equ  125
kGFMax:  .equ  126
EchoStart:  .equ  127
kListName:  .equ  127
kAdd:  .equ  128
kSub:  .equ  129
kMul:  .equ  130
kDiv:  .equ  131
kExpon:  .equ  132
kLParen:  .equ  133
kRParen:  .equ  134
kLBrack:  .equ  135
kRBrack:  .equ  136
kShade:  .equ  137
kStore:  .equ  138
kComma:  .equ  139
kChs:  .equ  140
kDecPnt:  .equ  141
k0:  .equ  142
k1:  .equ  143
k2:  .equ  144
k3:  .equ  145
k4:  .equ  146
k5:  .equ  147
k6:  .equ  148
k7:  .equ  149
k8:  .equ  150
k9:  .equ  151
kEE:  .equ  152
kSpace:  .equ  153
kCapA:  .equ  154
kCapB:  .equ  155
kCapC:  .equ  156
kCapD:  .equ  157
kCapE:  .equ  158
kCapF:  .equ  159
kCapG:  .equ  160
kCapH:  .equ  161
kCapI:  .equ  162
kCapJ:  .equ  163
kCapK:  .equ  164
kCapL:  .equ  165
kCapM:  .equ  166
kCapN:  .equ  167
kCapO:  .equ  168
kCapP:  .equ  169
kCapQ:  .equ  170
kCapR:  .equ  171
kCapS:  .equ  172
kCapT:  .equ  173
kCapU:  .equ  174
kCapV:  .equ  175
kCapW:  .equ  176
kCapX:  .equ  177
kCapY:  .equ  178
kCapZ:  .equ  179
kVarx:  .equ  180
kPi:  .equ  181
kInv:  .equ  182
kSin:  .equ  183
kASin:  .equ  184
kCos:  .equ  185
kACos:  .equ  186
kTan:  .equ  187
kATan:  .equ  188
kSquare:  .equ  189
kSqrt:  .equ  190
kLn:  .equ  191
kExp:  .equ  192
kLog:  .equ  193
kALog:  .equ  194
kToABC:  .equ  195
kClrTbl:  .equ  196
kAns:  .equ  197
kColon:  .equ  198
kNDeriv:  .equ  199
kFnInt:  .equ  200
kRoot:  .equ  201
kQuest:  .equ  202
kQuote:  .equ  203
kTheta:  .equ  204
kIf:  .equ  205
kThen:  .equ  206
kElse:  .equ  207
kFor:  .equ  208
kWhile:  .equ  209
kRepeat:  .equ  210
kEnd:  .equ  211
kPause:  .equ  212
kLbl:  .equ  213
kGoto:  .equ  214
kISG:  .equ  215
kDSL:  .equ  216
kMenu:  .equ  217
kExec:  .equ  218
kReturn:  .equ  219
kStop:  .equ  220
kInput:  .equ  221
kPrompt:  .equ  222
kDisp:  .equ  223
kDispG:  .equ  224
kDispT:  .equ  225
kOutput:  .equ  226
kGetKey:  .equ  227
kClrHome:  .equ  228
kPrtScr:  .equ  229
kSinH:  .equ  230
kCosH:  .equ  231
kTanH:  .equ  232
kASinH:  .equ  233
kACosH:  .equ  234
kATanH:  .equ  235
kLBrace:  .equ  236
kRBrace:  .equ  237
kI:  .equ  238
kCONSTeA:  .equ  239
kPlot3:  .equ  240
kFMin:  .equ  241
kFMax:  .equ  242
kL1A:  .equ  243
kL2A:  .equ  244
kL3A:  .equ  245
kL4A:  .equ  246
kL5A:  .equ  247
kL6A:  .equ  248
kunA:  .equ  249
kvnA:  .equ  250
kwnA:  .equ  251
kExtendEcho2:  .equ  252
kExtendEcho:  .equ  254
kE1BT:  .equ  0
kDrawInv:  .equ  0
kDrawF:  .equ  1
kPixelOn:  .equ  2
kPixelOff:  .equ  3
kPxlTest:  .equ  4
kRCGDB:  .equ  5
kRCPic:  .equ  6
kSTGDB:  .equ  7
kSTPic:  .equ  8
kAbs:  .equ  9
kTEqu:  .equ  10
kTNoteQ:  .equ  11
kTGT:  .equ  12
kTGTE:  .equ  13
kTLT:  .equ  14
kTLTE:  .equ  15
kAnd:  .equ  16
kOr:  .equ  17
kXor:  .equ  18
kNot:  .equ  19
kLR1:  .equ  20
kXRoot:  .equ  21
kCube:  .equ  22
kCbRt:  .equ  23
kToDec:  .equ  24
kCubicR:  .equ  25
kQuartR:  .equ  26
kPlot1:  .equ  27
kPlot2:  .equ  28
kRound:  .equ  29
kIPart:  .equ  30
kFPart:  .equ  31
kInt:  .equ  32
kRand:  .equ  33
kNPR:  .equ  34
kNCR:  .equ  35
kXFactorial:  .equ  36
kRad:  .equ  37
kDegr:  .equ  38
kAPost:  .equ  39
kToDMS:  .equ  40
kRToPo:  .equ  41
kRToPr:  .equ  42
kPToRx:  .equ  43
kPToRy:  .equ  44
kRowSwap:  .equ  45
kRowPlus:  .equ  46
kTimRow:  .equ  47
kTRowP:  .equ  48
kSortA:  .equ  49
kSortD:  .equ  50
kSeq:  .equ  51
kMin:  .equ  52
kMax:  .equ  53
kMean:  .equ  54
kMedian:  .equ  55
kSum:  .equ  56
kProd:  .equ  57
kDet:  .equ  58
kTransp:  .equ  59
kDim:  .equ  60
kFill:  .equ  61
kIdent:  .equ  62
kRandm:  .equ  63
kAug:  .equ  64
kOneVar:  .equ  65
kTwoVar:  .equ  66
kLR:  .equ  67
kLRExp:  .equ  68
kLRLn:  .equ  69
kLRPwr:  .equ  70
kMedMed:  .equ  71
kQuad:  .equ  72
kClrLst:  .equ  73
kHist:  .equ  74
kxyLine:  .equ  75
kScatter:  .equ  76
kmRad:  .equ  77
kmDeg:  .equ  78
kmNormF:  .equ  79
kmSci:  .equ  80
kmEng:  .equ  81
kmFloat:  .equ  82
kFix:  .equ  83
kSplitOn:  .equ  84
kFullScreen:  .equ  85
kStndrd:  .equ  86
kParam:  .equ  87
kPolar:  .equ  88
kSeqG:  .equ  89
kAFillOn:  .equ  90
kAFillOff:  .equ  91
kACalcOn:  .equ  92
kACalcOff:  .equ  93
kFNOn:  .equ  94
kFNOff:  .equ  95
kPlotsOn:  .equ  96
kPlotsOff:  .equ  97
kPixelChg:  .equ  98
kSendMBL:  .equ  99
kRecvMBL:  .equ  100
kBoxPlot:  .equ  101
kBoxIcon:  .equ  102
kCrossIcon:  .equ  103
kDotIcon:  .equ  104
kE2BT:  .equ  105
kSeqential:  .equ  105
kSimulG:  .equ  106
kPolarG:  .equ  107
kRectG:  .equ  108
kCoordOn:  .equ  109
kCoordOff:  .equ  110
kDrawLine:  .equ  111
kDrawDot:  .equ  112
kAxisOn:  .equ  113
kAxisOff:  .equ  114
kGridOn:  .equ  115
kGridOff:  .equ  116
kLblOn:  .equ  117
kLblOff:  .equ  118
kL1:  .equ  119
kL2:  .equ  120
kL3:  .equ  121
kL4:  .equ  122
kL5:  .equ  123
kL6:  .equ  124
kinfData:  .equ  119
kinfStats:  .equ  120
kinfYes:  .equ  121
kinfNo:  .equ  122
kinfCalc:  .equ  123
kinfDraw:  .equ  124
kinfAlt1ne:  .equ  125
kinfAlt1lt:  .equ  126
kinfAlt1gt:  .equ  127
kinfAlt2ne:  .equ  128
kinfAlt2lt:  .equ  129
kinfAlt2gt:  .equ  130
kinfAlt3ne:  .equ  131
kinfAlt3lt:  .equ  132
kinfAlt3gt:  .equ  133
kinfAlt4ne:  .equ  134
kinfAlt4lt:  .equ  135
kinfAlt4gt:  .equ  136
kinfAlt5ne:  .equ  137
kinfAlt5lt:  .equ  138
kinfAlt5gt:  .equ  139
kinfAlt6ne:  .equ  140
kinfAlt6lt:  .equ  141
kinfAlt6gt:  .equ  142
kMatA:  .equ  125
kMatB:  .equ  126
kMatC:  .equ  127
kMatD:  .equ  128
kMatE:  .equ  129
kXmin:  .equ  130
kXmax:  .equ  131
kXscl:  .equ  132
kYmin:  .equ  133
kYmax:  .equ  134
kYscl:  .equ  135
kTmin:  .equ  136
kTmax:  .equ  137
kTStep:  .equ  138
kOmin:  .equ  139
kOmax:  .equ  140
kOStep:  .equ  141
ku0:  .equ  142
kv0:  .equ  143
knMin:  .equ  144
knMax:  .equ  145
kDeltaY:  .equ  146
kDeltaX:  .equ  147
kZXmin:  .equ  148
kZXmax:  .equ  149
kZXscl:  .equ  150
kZYmin:  .equ  151
kZYmax:  .equ  152
kZYscl:  .equ  153
kZTmin:  .equ  154
kZTmax:  .equ  155
kZTStep:  .equ  156
kZOmin:  .equ  157
kZOmax:  .equ  158
kZOStep:  .equ  159
kZu0:  .equ  160
kZv0:  .equ  161
kZnMin:  .equ  162
kZnMax:  .equ  163
kDelLast:  .equ  164
kSinReg:  .equ  165
kConstE:  .equ  166
kPic1:  .equ  167
kPic2:  .equ  168
kPic3:  .equ  169
kDelVar:  .equ  170
kGetCalc:  .equ  171
kRealM:  .equ  172
kPolarM:  .equ  173
kRectM:  .equ  174
kuv:  .equ  175
kvw:  .equ  176
kuw:  .equ  177
kFinPMTend:  .equ  178
kFinPMTbeg:  .equ  179
kGraphStyle:  .equ  180
kExprOn:  .equ  181
kExprOff:  .equ  182
kStatA:  .equ  183
kStatB:  .equ  184
kStatC:  .equ  185
kCorr:  .equ  186
kStatD:  .equ  187
kStatE:  .equ  188
kRegEq:  .equ  189
kMinX:  .equ  190
kQ1:  .equ  191
kMD:  .equ  192
kQ3:  .equ  193
kMaxX:  .equ  194
kStatX1:  .equ  195
kStatY1:  .equ  196
kStatX2:  .equ  197
kStatY2:  .equ  198
kStatX3:  .equ  199
kStatY3:  .equ  200
kTblMin:  .equ  201
kTblStep:  .equ  202
kSetupLst:  .equ  203
kClrAllLst:  .equ  204
kLogistic:  .equ  205
kZTest:  .equ  206
kTTest:  .equ  207
k2SampZTest:  .equ  208
k2SampTTest:  .equ  209
k1PropZTest:  .equ  210
k2PropZTest:  .equ  211
kChiTest:  .equ  212
k2SampFTest:  .equ  213
kZIntVal:  .equ  214
kTIntVal:  .equ  215
k2SampTInt:  .equ  216
k2SampZInt:  .equ  217
k1PropZInt:  .equ  218
k2PropZInt:  .equ  219
kDNormal:  .equ  220
kInvNorm:  .equ  221
kDT:  .equ  222
kChi:  .equ  223
kDF:  .equ  224
kBinPDF:  .equ  225
kBinCDF:  .equ  226
kPoiPDF:  .equ  227
kPoiCDF:  .equ  228
kun:  .equ  229
kvn:  .equ  230
kwn:  .equ  231
kRecn:  .equ  232
kPlotStart:  .equ  233
kZPlotStart:  .equ  234
kXFact:  .equ  235
kYFact:  .equ  236
kANOVA:  .equ  237
kMaxY:  .equ  238
kWebOn:  .equ  239
kWebOff:  .equ  240
kTblInput:  .equ  241
kGeoPDF:  .equ  242
kGeoCDF:  .equ  243
kShadeNorm:  .equ  244
kShadeT:  .equ  245
kShadeChi:  .equ  246
kShadeF:  .equ  247
kPlotStep:  .equ  248
kZPlotStep:  .equ  249
kLinRegtTest:  .equ  250
kMGT:  .equ  251
kSelectA:  .equ  152
kZFitA:  .equ  253
kE2BT_End:  .equ  253
kE2BT2:  .equ  0
kGDB1:  .equ  0
kGDB2:  .equ  1
kGDB3:  .equ  2
kY1:  .equ  3
kY2:  .equ  4
kY3:  .equ  5
kY4:  .equ  6
kY5:  .equ  7
kY6:  .equ  8
kY7:  .equ  9
kY8:  .equ  10
kY9:  .equ  11
kY0:  .equ  12
kX1T:  .equ  13
kY1T:  .equ  14
kX2T:  .equ  15
kY2T:  .equ  16
kX3T:  .equ  17
kY3T:  .equ  18
kX4T:  .equ  19
kY4T:  .equ  20
kX5T:  .equ  21
kY5T:  .equ  22
kX6T:  .equ  23
kY6T:  .equ  24
kR1:  .equ  25
kR2:  .equ  26
kR3:  .equ  27
kR4:  .equ  28
kR5:  .equ  29
kR6:  .equ  30
kGDB4:  .equ  31
kGDB5:  .equ  32
kGDB6:  .equ  33
kPic4:  .equ  34
kPic5:  .equ  35
kPic6:  .equ  36
kGDB7:  .equ  37
kGDB8:  .equ  38
kGDB9:  .equ  39
kGDB0:  .equ  40
kPic7:  .equ  41
kPic8:  .equ  42
kPic9:  .equ  43
kPic0:  .equ  44
kStatN:  .equ  45
kXMean:  .equ  46
kConj:  .equ  47
kReal:  .equ  48
kFAngle:  .equ  49
kLCM:  .equ  50
kGCD:  .equ  51
kRandInt:  .equ  52
kRandNorm:  .equ  53
kToPolar:  .equ  54
kToRect:  .equ  55
kYMean:  .equ  56
kStdX:  .equ  57
kStdX1:  .equ  58
kw0:  .equ  59
kMatF:  .equ  60
kMatG:  .equ  61
kMatRH:  .equ  62
kMatI:  .equ  63
kMatJ:  .equ  64
kYMean1:  .equ  65
kStdY:  .equ  66
kStdY1:  .equ  67
kMatToLst:  .equ  68
kLstToMat:  .equ  69
kCumSum:  .equ  70
kDeltaLst:  .equ  71
kStdDev:  .equ  72
kVariance:  .equ  73
kLength:  .equ  74
kEquToStrng:  .equ  75
kStrngToEqu:  .equ  76
kExpr:  .equ  77
kSubStrng:  .equ  78
kInStrng:  .equ  79
kStr1:  .equ  80
kStr2:  .equ  81
kStr3:  .equ  82
kStr4:  .equ  83
kStr5:  .equ  84
kStr6:  .equ  85
kStr7:  .equ  86
kStr8:  .equ  87
kStr9:  .equ  88
kStr0:  .equ  89
kFinN:  .equ  90
kFinI:  .equ  91
kFinPV:  .equ  92
kFinPMT:  .equ  93
kFinFV:  .equ  94
kFinPY:  .equ  95
kFinCY:  .equ  96
kFinFPMT:  .equ  97
kFinFI:  .equ  98
kFinFPV:  .equ  99
kFinFN:  .equ  100
kFinFFV:  .equ  101
kFinNPV:  .equ  102
kFinIRR:  .equ  103
kFinBAL:  .equ  104
kFinPRN:  .equ  105
kFinINT:  .equ  106
kSumX:  .equ  107
kSumX2:  .equ  108
kFinToNom:  .equ  109
kFinToEff:  .equ  110
kFinDBD:  .equ  111
kStatVP:  .equ  112
kStatZ:  .equ  113
kStatT:  .equ  114
kStatChi:  .equ  115
kStatF:  .equ  116
kStatDF:  .equ  117
kStatPhat:  .equ  118
kStatPhat1:  .equ  119
kStatPhat2:  .equ  120
kStatMeanX1:  .equ  121
kStatMeanX2:  .equ  122
kStatStdX1:  .equ  123
kStatStdX2:  .equ  124
kStatStdXP:  .equ  125
kStatN1:  .equ  126
kStatN2:  .equ  127
kStatLower:  .equ  128
kStatUpper:  .equ  129
kuw0:  .equ  130
kImag:  .equ  131
kSumY:  .equ  132
kXres:  .equ  133
kStat_s:  .equ  134
kSumY2:  .equ  135
kSumXY:  .equ  136
kuXres:  .equ  137
kModBox:  .equ  138
kNormProb:  .equ  139
kNormalPDF:  .equ  140
kTPDF:  .equ  141
kChiPDF:  .equ  142
kFPDF:  .equ  143
kMinY:  .equ  144
kRandBin:  .equ  145
kRef:  .equ  146
kRRef:  .equ  147
kLRSqr:  .equ  148
kBRSqr:  .equ  149
kDiagOn:  .equ  150
kDiagOff:  .equ  151
kun1:  .equ  152
kvn1:  .equ  153
k83_00End:  .equ  153
kArchive:  .equ  154
kUnarchive:  .equ  155
kAsm:  .equ  156
kAsmPrgm:  .equ  157
kAsmComp:  .equ  158
kcapAAcute:  .equ  159
kcapAGrave:  .equ  160
kcapACaret:  .equ  161
kcapADier:  .equ  162
kaAcute:  .equ  163
kaGrave:  .equ  164
kaCaret:  .equ  165
kaDier:  .equ  166
kcapEAcute:  .equ  167
kcapEGrave:  .equ  168
kcapECaret:  .equ  169
kcapEDier:  .equ  170
keAcute:  .equ  171
keGrave:  .equ  172
keCaret:  .equ  173
keDier:  .equ  174
kcapIAcute:  .equ  175
kcapIGrave:  .equ  176
kcapICaret:  .equ  177
kcapIDier:  .equ  178
kiAcute:  .equ  179
kiGrave:  .equ  180
kiCaret:  .equ  181
kiDier:  .equ  182
kcapOAcute:  .equ  183
kcapOGrave:  .equ  184
kcapOCaret:  .equ  185
kcapODier:  .equ  186
koAcute:  .equ  187
koGrave:  .equ  188
koCaret:  .equ  189
koDier:  .equ  190
kcapUAcute:  .equ  191
kcapUGrave:  .equ  192
kcapUCaret:  .equ  193
kcapUDier:  .equ  194
kuAcute:  .equ  195
kuGrave:  .equ  196
kuCaret:  .equ  197
kuDier:  .equ  198
kcapCCed:  .equ  199
kcCed:  .equ  200
kcapNTilde:  .equ  201
knTilde:  .equ  202
kaccent:  .equ  203
kgrave:  .equ  204
kdieresis:  .equ  205
kquesDown:  .equ  206
kexclamDown:  .equ  207
kalpha:  .equ  208
kbeta:  .equ  209
kgamma:  .equ  210
kcapDelta:  .equ  211
kdelta:  .equ  212
kepsilon:  .equ  213
klambda:  .equ  214
kmu:  .equ  215
kpi2:  .equ  216
krho:  .equ  217
kcapSigma:  .equ  218
ksigma:  .equ  219
ktau:  .equ  220
kphi:  .equ  221
kcapOmega:  .equ  222
kphat:  .equ  223
kchi2:  .equ  224
kstatF2:  .equ  225
kLa:  .equ  226
kLb:  .equ  227
kLc:  .equ  228
kLd:  .equ  229
kLe:  .equ  230
kLf:  .equ  231
kLg:  .equ  232
kLh:  .equ  233
kLi:  .equ  234
kLj:  .equ  235
kLk:  .equ  236
kLl:  .equ  237
kLm:  .equ  238
kLsmalln:  .equ  239
kLo:  .equ  240
kLp:  .equ  241
kLq:  .equ  242
kLsmallr:  .equ  243
kLs:  .equ  244
kLt:  .equ  245
kLu:  .equ  246
kLv:  .equ  247
kLw:  .equ  248
kLx:  .equ  249
kLy:  .equ  250
kLz:  .equ  251
kGarbageC:  .equ  252
kE2BT2_End:  .equ  252
KE2BT3:  .equ  0
kReserved:  .equ  1
kAtSign:  .equ  2
kPound:  .equ  3
kDollar:  .equ  4
kAmpersand:  .equ  5
kBackQuote:  .equ  6
kSemicolon:  .equ  7
kBackSlash:  .equ  8
kVertSlash:  .equ  9
kUnderscore:  .equ  10
kTilde:  .equ  11
kPercent:  .equ  12
kLastUsedK3:  .equ  12
kTab:  .equ  13
kShftTaB:  .equ  14
kShftDel:  .equ  15
kShftBack:  .equ  16
kShftPgUp:  .equ  17
kShftPgDn:  .equ  18
kShftLeft:  .equ  19
kShftRight:  .equ  20
kShftUp:  .equ  21
kShftDn:  .equ  22
kDiamond:  .equ  23
kDiaAdd:  .equ  23
kDiaSub:  .equ  24
kDiaTilde:  .equ  25
kDiaDiv:  .equ  26
kDiaBkSlash:  .equ  27
kDiaColon:  .equ  28
kDiaQuote:  .equ  29
kDiaLBrack:  .equ  30
num:  .equ  0
str:  .equ  1
char:  .equ  2
NumRight:  .equ  11
    LD HL, main_mymenu
    PUSH HL
    LD HL, 0
    PUSH HL
    LD HL, main_mtitle
    PUSH HL
    CALL menuoption
    LD HL, main_mymenu
    PUSH HL
    LD HL, 1
    PUSH HL
    LD HL, main_mop1
    PUSH HL
    CALL menuoption
    LD HL, main_mymenu
    PUSH HL
    LD HL, 2
    PUSH HL
    LD HL, main_mop2
    PUSH HL
    CALL menuoption
__LOOP__A__BEGIN__:
    LD HL, main_mymenu
    PUSH HL
    LD HL, 2
    PUSH HL
    CALL menu
    LD (main_result), HL
    LD HL, (main_result)
    PUSH HL
    LD HL, 1
    POP DE
    LD A, H
    CP D
    JP NZ, __IF__B__TEST__
    LD A, L
    CP E
    JP NZ, __IF__B__TEST__
__IF__B__TEMP__:
    CALL cls
    CALL count
    LD HL, 2
    PUSH HL
    CALL sleep
    CALL cls
    JP __IF__B__END__
__IF__B__TEST__:
    LD HL, 5
    PUSH HL
    CALL sleep
    CALL cls
    LD SP, (__RETPointer)
    RET
__IF__B__END__:
    JP __LOOP__A__BEGIN__
__LOOP__A__TEST__:
    RET

cls:
    RST $28
.DW $4546
    LD HL, 0
    LD (curRow), HL
    RET

sleep:
    POP BC
    POP HL
    LD D, L
    PUSH BC
    DEC D
    LD E, $FF
sleep_loop1:
    LD B, $9F
sleep_loop2:
    DJNZ sleep_loop2
    DEC DE
    LD A, D
    OR E
    JP NZ, sleep_loop1
    RET

menu:
    POP BC
    POP HL
    LD A, L
    LD (menu_display__entry_num), A
    POP HL
    LD (menu_display__dataptr), HL
    PUSH BC
menu_display:
    SET plotLoc, (IY + plotFlags)
    LD HL, $0808
    LD DE, $3857
    RST $28
.DW $4D5C
    LD HL, $0909
    LD DE, $3656
    RST $28
.DW $4D7D
    LD BC, $0A08
    LD DE, $5708
    RST $28
.DW $47E0
    LD BC, $5708
    LD DE, $5735
    RST $28
.DW $47E0
    LD BC, $0A2E
    LD A, 38
    LD D, 1
menu_display_pointdisp:
    DEC A
    RST $28
.DW $47E3
    INC B
    INC B
    AND A
    JP NZ, menu_display_pointdisp
    LD HL, $0A0B
    LD (PenCol), HL
    LD HL, menu_display__entry_num
    LD A, (HL)
    LD HL, menu_display__entry_end
    LD (HL), A
    CP 5
    JP C, menu_display_resumedisp
    LD (HL), 5
menu_display_resumedisp:
    LD HL, (menu_display__dataptr)
    CALL menu_display_Goto_address
    RST $28
.DW $4561
    LD HL, (menu_display__dataptr)
    INC HL
    INC HL
    LD (menu_display__dataptr), HL
    LD HL, PenCol + 1
    INC (HL)
    INC (HL)
    CALL menu_display_Clear
    LD HL, menu_display__cur_entry
    LD (HL), 1
    LD HL, menu_display__top_entry
    LD (HL), 1
    LD HL, (menu_display__dataptr)
    CALL menu_display_Choicedisp
    LD HL, $130A
    LD (menu_display__cursor_coord), HL
    CALL menu_display_Invert
menu_display_keyloop:
    LD A, $FE
    OUT (1), A
    NOP
    NOP
    IN A, (1)
    BIT 0, A
    JP Z, menu_display_down
    BIT 3, A
    JP Z, menu_display_up
menu_display_resume_keyloop:
    LD A, $FD
    OUT (1), A
    NOP
    NOP
    IN A, (1)
    BIT 0, A
    JP Z, menu_display_enterkey
    LD HL, menu_display_exit
    PUSH HL
    BIT 6, A
    JP Z, menu_display_clearkey
    LD A, $EF
    OUT (1), A
    NOP
    NOP
    IN A, (1)
    BIT 0, A
    JP Z, menu_display_key0
    BIT 1, A
    JP Z, menu_display_key1
    BIT 2, A
    JP Z, menu_display_key4
    BIT 3, A
    JP Z, menu_display_key7
    LD A, $F7
    OUT (1), A
    NOP
    NOP
    IN A, (1)
    BIT 1, A
    JP Z, menu_display_key2
    BIT 2, A
    JP Z, menu_display_key5
    BIT 3, A
    JP Z, menu_display_key8
    LD A, $FB
    OUT (1), A
    NOP
    NOP
    IN A, (1)
    BIT 1, A
    JP Z, menu_display_key3
    BIT 2, A
    JP Z, menu_display_key6
    BIT 3, A
    JP Z, menu_display_key9
    POP AF
    JP menu_display_keyloop
menu_display_clearkey:
    LD B, - 1
    RET
menu_display_key0:
    LD B, 10
    RET
menu_display_key1:
    LD B, 1
    RET
menu_display_key2:
    LD B, 2
    RET
menu_display_key3:
    LD B, 3
    RET
menu_display_key4:
    LD B, 4
    RET
menu_display_key5:
    LD B, 5
    RET
menu_display_key6:
    LD B, 6
    RET
menu_display_key7:
    LD B, 7
    RET
menu_display_key8:
    LD B, 8
    RET
menu_display_key9:
    LD B, 9
    RET
menu_display_enterkey:
    LD HL, menu_display__cur_entry
    LD B, (HL)
menu_display_exit:
    RES plotLoc, (IY + plotFlags)
    LD A, $FF
    OUT (1), A
    NOP
    NOP
    IN A, (1)
    LD A, B
    LD L, A
    LD H, 0
    CP $FF
    RET NZ
    LD H, $FF
    RET
menu_display_down:
    LD HL, menu_display__entry_num
    LD A, (HL)
    LD HL, menu_display__cur_entry
    CP (HL)
    JP Z, menu_display_resume_keyloop
    INC (HL)
    CALL menu_display_Invert
    CALL menu_display_Comp_c_t_entry
    CP 4
    JP Z, menu_display_more
    CALL menu_display_Slowdown
    LD HL, (menu_display__cursor_coord)
    LD A, 7
    ADD A, H
    LD H, A
    LD (menu_display__cursor_coord), HL
    JP menu_display_resume_pre_key
menu_display_resume_pre_key:
    CALL menu_display_Invert
    JP menu_display_resume_keyloop
menu_display_up:
    LD HL, menu_display__cur_entry
    LD A, (HL)
    CP 1
    JP Z, menu_display_resume_keyloop
    DEC (HL)
    CALL menu_display_Invert
    CALL menu_display_Comp_c_t_entry
    CP - 2
    JP Z, menu_display_less
    CALL menu_display_Slowdown
    LD HL, (menu_display__cursor_coord)
    LD A, - 7
    ADD A, H
    LD H, A
    LD (menu_display__cursor_coord), HL
    JP menu_display_resume_pre_key
menu_display_Slowdown:
    LD DE, $2000
menu_display_slowloop:
    DEC DE
    LD A, D
    AND A
    JP NZ, menu_display_slowloop
    RET
menu_display_Comp_c_t_entry:
    LD HL, menu_display__top_entry
    LD A, (HL)
    CPL
    LD HL, menu_display__cur_entry
    ADD A, (HL)
    RET
menu_display_Clear:
    LD HL, $130B
    LD DE, $3555
    RST $28
.DW $4D5C
    RET
menu_display_Invert:
    LD HL, (menu_display__cursor_coord)
    LD D, H
    LD A, 6
    ADD A, D
    LD D, A
    LD E, $55
    RST $28
.DW $4D5F
    RET
menu_display_less:
    LD HL, menu_display__top_entry
    DEC (HL)
    INC HL
    DEC (HL)
    JP menu_display_mainml
menu_display_more:
    LD HL, menu_display__top_entry
    INC (HL)
    INC HL
    INC (HL)
menu_display_mainml:
    CALL menu_display_Clear
    LD HL, $0C0B
    LD (PenCol), HL
    LD HL, menu_display__top_entry
    LD E, (HL)
    DEC E
    LD D, 0
    LD L, E
    LD H, 0
    ADD HL, DE
    LD E, L
    LD D, H
    LD HL, (menu_display__dataptr)
    ADD HL, DE
    CALL menu_display_Choicedisp
    JP menu_display_resume_pre_key
menu_display_Choicedisp:
    EX DE, HL
    LD HL, menu_display__top_entry
    LD A, (HL)
    DEC A
    LD HL, menu_display__temp
    LD (HL), A
    EX DE, HL
menu_display_disploop:
    PUSH HL
    CALL menu_display_Goto_address
    PUSH HL
    LD HL, PenCol
    LD (HL), $0B
    LD A, 7
    INC HL
    ADD A, (HL)
    LD (HL), A
    LD HL, menu_display__temp
    LD A, (HL)
    INC A
    LD (HL), A
    CALL menu_display_Disp_preface
    POP HL
    RST $28
.DW $4561
    LD HL, menu_display__entry_end
    LD B, (HL)
    INC HL
    LD A, (HL)
    POP HL
    INC HL
    INC HL
    CP B
    JP NZ, menu_display_disploop
    RET
menu_display_Goto_address:
    LD B, (HL)
    INC HL
    LD C, (HL)
    LD H, C
    LD L, B
    RET
menu_display_Disp_preface:
    LD B, 0
    CP 11
    JP C, menu_display_continue_preface
    XOR A
    LD B, 1
menu_display_continue_preface:
    LD E, A
    LD D, 0
    LD IX, menu_display__num_preface
    ADD IX, DE
    LD HL, menu_display__2ndhalf_preface
    LD A, (IX+0)
    LD (HL), A
    LD A, B
    AND A
    JP Z, menu_display_res_disp_pref
    LD HL, menu_display__begin_preface
menu_display_res_disp_pref:
    RST $28
.DW $4561
    RET

menuoption:
    POP BC
    POP HL
    LD D, H
    LD E, L
    POP HL
    LD A, L
    POP HL
    PUSH BC
    CP 0
    JP Z, menuoption_cont
    LD B, A
menuoption_again:
    INC HL
    INC HL
    DJNZ menuoption_again
menuoption_cont:
    LD (HL), E
    INC HL
    LD (HL), D
    RET

count:
    LD HL, 100
    LD (count_i), HL
    LD HL, 0
    DEC HL
    PUSH HL
__FORNEXT__C__BEGIN__:
    LD HL, 0
    PUSH HL
    LD HL, 11
    PUSH HL
    CALL locate
    LD HL, num
    PUSH HL
    LD HL, (count_i)
    PUSH HL
    CALL printf
    LD HL, (count_i)
    DEC HL
    LD (count_i), HL
    POP DE
    DEC SP
    DEC SP
    LD A, D
    CP H
    JP NZ, __FORNEXT__C__BEGIN__
    LD A, E
    CP L
    JP NZ, __FORNEXT__C__BEGIN__
    POP HL
    RET

locate:
    POP BC
    POP HL
    LD A, L
    LD ($844C), A
    POP HL
    LD A, L
    LD ($844B), A
    PUSH BC
    RET

printf:
    POP BC
    POP HL
    LD D, H
    LD E, L
    POP HL
    LD A, L
    PUSH BC
    EX DE, HL
    CP 1
    JP Z, printf_string
    CP 2
    JP Z, printf_char
    RST $28
.DW $4507
    JP printf_end
printf_string:
    RST $28
.DW $450A
    JP printf_end
printf_char:
    LD A, L
    CALL putc
printf_end:
    RST $28
.DW $452E
    RET

putc:
    POP BC
    POP HL
    PUSH BC
    RST $28
.DW $4504
    RET

__RETPointer:
.DW 0

menu_display__entry_num:
.DB 0

main_mtitle:
.DB 84
.DB 101
.DB 115
.DB 116
.DB 32
.DB 77
.DB 101
.DB 110
.DB 117
.DB 58
.DB 0

main_mop1:
.DB 67
.DB 111
.DB 117
.DB 110
.DB 116
.DB 0

main_mop2:
.DB 69
.DB 120
.DB 105
.DB 116
.DB 0

main_result:
.DW 0

menu_display__dataptr:
.DW 0

menu_display__cursor_coord:
.DW 0

menu_display__cur_entry:
.DB 0

menu_display__top_entry:
.DB 0

menu_display__entry_end:
.DB 0

menu_display__temp:
.DB 0

menu_display__num_preface:
.DB 32
.DB 49
.DB 50
.DB 51
.DB 52
.DB 53
.DB 54
.DB 55
.DB 56
.DB 57
.DB 48
.DB 0

menu_display__begin_preface:
.DB 32
.DB 32
.DB 32
.DB 0

menu_display__2ndhalf_preface:
.DB 32
.DB 58
.DB 32
.DB 32
.DB 0

count_i:
.DW 0

main_mymenu:
.DW 0
.DW 0
.DW 0
